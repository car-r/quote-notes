...
export default function Sidebar({toggle, isOpen}: any) {
    const links = [
        ...
        {title: 'Content', route: '/content', icon: <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
            ...
            <ul className="flex flex-col gap-4">
                {links.map((link) => (
                    <NavLink to={`${link.route}`} key={link.title}
                        className={({ isActive }) =>
                        `flex py-1 px-3 rounded-md hover:border-blue-400 hover:text-stone-100 ${isActive ? "bg-stone-800 border  border-blue-400 text-stone-100" : "border border-stone-900"}`
                        }
                    >
                        <div>{link.icon}</div>
