    ...
    return (
        <>
            ...
            <div className="prose md:prose-lg lg:prose-xl flex flex-col mx-auto max-w-4xl py-20 prose-img:mb-0 prose-img:mt-0 prose-a:no-underline">
                {posts.map((post: any) => (
                    <div  key={post.slug} className=" pb-10 border-b border-b-stone-700">
                        <Link to={`/blog/${post.slug}`} className="flex flex-col md:flex-row text-stone-200 justify-center md:gap-10" >
                            <img src={post.featureImage} alt="" className="md:w-1/2"/>
                            <div className="flex flex-col">
                                <p className="md:pt-4 hover:text-blue-400">{post.title}</p>
                                <p className="text-base">{post.readMore}</p>
